...
| 2026-08-11 | `assessments`/`symptom_responses` migration (`c243bf08b89a`) was hand-written instead of via `alembic revision --autogenerate`. | No `.env` present in this environment, so `Settings()`/Alembic can't connect to the DB. Run `alembic upgrade head` locally to apply it. |
